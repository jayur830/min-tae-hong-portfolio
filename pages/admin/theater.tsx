...
const Theater: NextPage = () => {
	...
	return (
		<section className="theater">
					...
					{(theaterState[year] as any[]).map((obj: any, j: number) => (
						<React.Fragment key={j}>
							<div className="theater-block">
								...
								{obj.scenes.length === 0 ? null : <SceneSlide type="theater" year={year} i={j} scenePage={obj.scenePage} scenes={obj.scenes} setScene={setTheaterScene} />}
							</div>
							<div>
								<BlackButton
									onClick={() => {
			...
			{theaterScene.year !== "" && theaterScene.theaterIndex !== -1 && theaterScene.sceneIndex !== -1 && theaterScene.max !== -1 ? (
				<Scene
					...
					contents={[
						...
						},
						{
							...
							component: (
								<input
									type="text"
									defaultValue={contentData.theater}
									onKeyUp={(e: any) => {
										if (e.key === "Enter") commitTheaterData();
										else {
											const _contentData = { ...contentData };
						...
						},
						{
							...
							component: (
								<input
									type="text"
									defaultValue={contentData.schedule}
									onKeyUp={(e: any) => {
										if (e.key === "Enter") commitTheaterData();
										else {
											const _contentData = { ...contentData };
