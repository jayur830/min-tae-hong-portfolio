...
import { useTheatersData, useSetVisibleModifyModal, useSetSelectedData, useOnRemoveByYear, useOnRemove } from '../Provider';

export default function List() {
	const theatersData = useTheatersData();
	...
	const getContents = useCallback(() => {
		return Object.keys(theatersData)
			.reverse()
			.map(year => {
				return {
					year,
					contents: nvl(theatersData, year, []).map(({ __typename, ...etc }: any) => etc),
				};
			});
	}, [theatersData]);
												...
												<Row gutter={[15, 0]}>
													<Col span={12}>
														<Descriptions column={1}>
															{nvl(values, 'adminTheatersSubmitModalValue.descriptions', [])
